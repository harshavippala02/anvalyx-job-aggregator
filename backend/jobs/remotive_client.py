...
def is_us_or_remote(location: str, description: str = ""):
...
def make_external_id(url: str):
    return "remotive_" + hashlib.md5(url.encode("utf-8")).hexdigest()


def fetch_remotive_jobs():
    try:
        res = requests.get(REMOTIVE_URL, timeout=60)
        res.raise_for_status()
        payload = res.json()
    except Exception as e:
        print(f"❌ Remotive fetch failed: {e}", flush=True)
        return []
    ...
    for job in raw_jobs:
        ...
        location = normalize_spaces(job.get("candidate_required_location")) or "Remote"
        ...
        if not is_us_or_remote(location, description):
            continue
    ...
    print(f"✅ Remotive fetched {len(jobs)} filtered jobs", flush=True)
    return jobs